...
//ultrapasse a cidade B em população. Claro que ela quer saber apenas para as cidades cuja taxa de crescimento da
//cidade A é maior do que a taxa de crescimento da cidade B, portanto, previamente já separou para você apenas os casos
//de teste que tem a taxa de crescimento maior para a cidade A. Sua tarefa é construir um programa que apresente o
//tempo em anos para cada caso de teste.
...
//caso de teste contém 4 números: dois inteiros PA e PB (100 ≤ PA < 1000000, PA < PB ≤ 1000000) indicando respectivamente
//a população de A e B, e dois valores G1 e G2 (0.1 ≤ G1 ≤ 10.0, 0.0 ≤ G2 ≤ 10.0, G2 < G1) com um digito após o
//ponto decimal cada, indicando respectivamente o crescimento populacional de A e B (em percentual).
...
class URI
{
    static void Main(string[] args)
    {
        ...
        for (int i = 0; i < T; ++i)
        {
            ...
            int pA = int.Parse(values[0]);
            int pB = int.Parse(values[1]);
            double g1 = double.Parse(values[2]) / 100;
            double g2 = double.Parse(values[3]) / 100;

            int years;
            for (years = 1; pA <= pB && years < 101; ++years)
            {
            ...
            {
                Console.WriteLine($"{years - 1} anos.");
            }
        }
    }
}